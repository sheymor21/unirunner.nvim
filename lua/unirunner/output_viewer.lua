...
local function setup_output_highlights()
  -- Define highlight groups that link to theme colors
  -- Using default=true so we don't override existing highlights
...
-- Render simple header - full green for success, red for failure
local function render_header()
  ...
  local function add_line(text, hl_group)
    ...
    line_num = line_num + 1
    if hl_group then
      table.insert(highlights, { line = line_num, col = 0, end_col = -1, hl_group = hl_group })
    end
  end
  
  -- Determine color based on status
  local status_hl = 'DiagnosticError'  -- default red
  if state.entry.status == 'success' then
    status_hl = 'DiagnosticOk'  -- green
  end
  
  -- Simple header with status color
  add_line('')
  add_line('  Command: ' .. state.entry.command)
  add_line('  Status: ' .. state.entry.status .. '  Duration: ' .. format_duration(state.entry.duration) .. '  Time: ' .. format_timestamp(state.entry.timestamp), status_hl)
  add_line('')
  add_line(string.rep('─', 80))
...
local function process_output(text)
  ...
  for line in text:gmatch('[^\n]*') do
    ...
    if #line == 0 or line:match('^%s*$') then
      ...
      -- Only keep max 2 consecutive empty lines
    ...
    -- Skip duplicate lines (dotnet shows many duplicate progress lines)
...
local function render_content()
  ...
  -- Highlight each key in brackets
  local pos = 3
